...
import { buildEsp32C3Image } from './image-builder.js';
...
export function createEsp32C3V2WorkerActionMessageHandler({
  dependencies = DEFAULT_DEPENDENCIES,
  postMessage,
} = {}) {
  if (typeof postMessage !== 'function') throw new TypeError('ESP32-C3 v2 Action postMessage is required');
  return createEsp32C3WorkerActionMessageHandler({
    openSession: async (request) => {
      const executor = await createEsp32BrowserActionExecutor({ init: request, dependencies });
      return {
        runAction: (action, context) => executor.execute(
          action,
          [...context.inputs.entries()].map(([path, bytes]) => ({ path, bytes })),
        ),
...
if (scope?.addEventListener && scope?.postMessage) {
  ...
  const actionHandler = createEsp32C3V2WorkerActionMessageHandler({ postMessage: post });
