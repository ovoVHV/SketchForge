...
import {
  createEsp32WorkerActionMessageHandler,
} from '../v1/c3-runtime.js';
import { buildEsp32Image } from './image-builder.js';
...
export function createEsp32V2WorkerActionMessageHandler({ dependencies = DEFAULT_DEPENDENCIES, postMessage } = {}) {
  if (typeof postMessage !== 'function') throw new TypeError('ESP32 v2 Action postMessage is required');
  return createEsp32WorkerActionMessageHandler({
...
if (scope?.addEventListener && scope?.postMessage) {
  ...
  const actionHandler = createEsp32V2WorkerActionMessageHandler({ postMessage: post });
