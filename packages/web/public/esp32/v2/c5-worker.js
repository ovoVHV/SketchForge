...
import {
  createEsp32C5WorkerActionMessageHandler,
} from '../v1/c3-runtime.js';
import { buildEsp32C5Image } from './image-builder.js';
...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  preprocess,
  buildImage: buildEsp32C5Image,
});

export function createEsp32C5V2WorkerActionMessageHandler({ dependencies = DEFAULT_DEPENDENCIES, postMessage } = {}) {
  if (typeof postMessage !== 'function') throw new TypeError('ESP32-C5 v2 Action postMessage is required');
  return createEsp32C5WorkerActionMessageHandler({
...
if (scope?.addEventListener && scope?.postMessage) {
  ...
  const actionHandler = createEsp32C5V2WorkerActionMessageHandler({ postMessage: post });
