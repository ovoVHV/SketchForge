...
import {
  createEsp32S2WorkerActionMessageHandler,
} from '../v1/c3-runtime.js';
import { buildEsp32S2Image } from './image-builder.js';
...
export function createEsp32S2V2WorkerActionMessageHandler({ dependencies = DEFAULT_DEPENDENCIES, postMessage } = {}) {
  if (typeof postMessage !== 'function') throw new TypeError('ESP32-S2 v2 Action postMessage is required');
  return createEsp32S2WorkerActionMessageHandler({
...
if (scope?.addEventListener && scope?.postMessage) {
  ...
  const actionHandler = createEsp32S2V2WorkerActionMessageHandler({ postMessage: post });
