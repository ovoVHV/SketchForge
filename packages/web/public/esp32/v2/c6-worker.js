...
import {
  createEsp32C6WorkerActionMessageHandler,
} from '../v1/c3-runtime.js';
import { buildEsp32C6Image } from './image-builder.js';
...
export function createEsp32C6V2WorkerActionMessageHandler({ dependencies = DEFAULT_DEPENDENCIES, postMessage } = {}) {
  if (typeof postMessage !== 'function') throw new TypeError('ESP32-C6 v2 Action postMessage is required');
  return createEsp32C6WorkerActionMessageHandler({
...
if (scope?.addEventListener && scope?.postMessage) {
  ...
  const actionHandler = createEsp32C6V2WorkerActionMessageHandler({ postMessage: post });
