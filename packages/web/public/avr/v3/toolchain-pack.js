...
export function createBrowserToolchainPackLoader({
  ...
  const loadManifest = () => {
    if (!manifestPromise) {
      const pending = fetchJson(fetchFn, url, "browser toolchain manifest", BROWSER_TOOLCHAIN_PACK_MANIFEST_MAX_BYTES)
        .then(async (value) => {
...
async function loadArtifactChunks({
  ...
  for (let index = 0; index < artifact.chunks.length; index++) {
    ...
    const downloaded = await fetchVerifiedBytes({
      fetchFn,
      url: resolveArtifactUrl(baseUrl, chunk.path),
...
async function fetchVerifiedBytes({
  ...
  for (const cache of ["force-cache", "reload"]) {
    try {
      ...
      lastError = error;
      if (cache === "reload" || !shouldRetryFetch(error)) throw error;
    }
  }
  throw lastError;
}
