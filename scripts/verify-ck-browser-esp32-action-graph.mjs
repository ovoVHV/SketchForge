...
const runtimeRoot = resolve(process.env.CK_RUNTIME_ROOT ?? 'var/work/runtime-v2-esp32-staging');
const descriptorUrl = pathToFileURL(resolve(process.env.CK_RUNTIME_DESCRIPTOR ?? resolve(runtimeRoot, 'runtime.json')));
const compilerBundleUrl = pathToFileURL(resolve(runtimeRoot, 'clang/bundle.js'));
...
const { buildEsp32Image } = await import('../packages/web/public/esp32/v2/image-builder.js');
...
async function loadBrowserToolchain(loader) {
  ...
  try {
    ...
    return await loadEsp32C3Toolchain({ loader, bundleUrl: compilerBundleUrl });
  } finally {
...
const capability = {
  profile: { board: descriptor.board, architecture: 'xtensa', runtime: 'esp32-xtensa', imageBuilder: true },
